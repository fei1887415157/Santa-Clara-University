...
void *getElements(SET *sp) {
    ...
    // interchangable: sp->count = elts.length
    QuickSort (elts, 0, sp->count - 1, sp);
...
of pivot */
int Partition (void** arr, int low, int high, SET *sp)
{ 

    assert (arr != NULL && low >= 0 && high < sp->count && sp != NULL);
    ...
    // Not inside for loop. Swap last with separate.
    temp = arr [high];
    arr [high] = arr [separate];
    arr [separate] = temp;
...
low --> Starting index, 
high --> Ending index */
void QuickSort(void** arr, int low, int high, SET *sp) 
{ 
    assert (arr != NULL && low >= 0 && high < sp->count && sp != NULL);

    if (low < high) { 
        /* partitioning index, arr[p] is now 
        at right place */
        int index = Partition(arr, low, high, sp); 
  
        // Separately sort elements before 
        // partition and after partition 
        QuickSort(arr, low, index - 1, sp); 
        QuickSort(arr, index + 1, high, sp); 
    } 
